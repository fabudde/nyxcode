 ...
 *   nyx check <file.nyx>    — Validate without emitting (use --strict to fail on warnings)
 ...
 */
/**
 * Pinned, known-good version ranges for every dependency the generated backend
 * can `require()`. Kept in one place so a generated app installs reproducibly.
 */
const BACKEND_DEP_VERSIONS: Record<string, string> = {
  express: '^4.21.2',
...
/**
 * Emit a runnable package.json (+ a short README) next to the generated server.js.
 *
 * Before this, `nyx build` wrote a server.js that `require()`d express / better-sqlite3 /
 * bcryptjs etc. but never declared them, so the backend could not be started without the
 * user hand-authoring a manifest. Emitting the manifest makes the output directory a
 * self-contained, runnable Node app: `npm install && npm start`.
 */
function emitBackendManifest(outDir: string, appName: string, deps: string[]): void {
  const dependencies: Record<string, string> = {};
  for (const d of deps) {
    dependencies[d] = BACKEND_DEP_VERSIONS[d] ?? 'latest';
  }
  // Sanitise the name to a valid npm package identifier.
  const safeName = (appName || 'nyxcode-app')
    .toLowerCase()
    .replace(/[^a-z0-9._-]+/g, '-')
    .replace(/^[-_.]+|[-_.]+$/g, '') || 'nyxcode-app';
  const pkg = {
    name: safeName,
    version: '1.0.0',
    private: true,
    description: `Full-stack app generated from ${appName}.nyx by NyxCode`,
    type: 'commonjs',
    main: 'server.js',
    scripts: {
  ...
  if (fs.existsSync(pkgPath)) {
    try {
      const existing = JSON.parse(readFileSync(pkgPath, 'utf8'));
      existing.dependencies = { ...dependencies, ...(existing.dependencies || {}) };
      existing.scripts = { start: 'node server.js', ...(existing.scripts || {}) };
      if (!existing.main) existing.main = 'server.js';
      writeFileSync(pkgPath, JSON.stringify(existing, null, 2) + '\n');
      return;
    } catch {
      // Fall through and overwrite a corrupt manifest.
    }
  }
...
Full-stack app generated by [NyxCode](https://nyxcode.io).
...
The server starts on \`http://localhost:3000\` (override with \`PORT=8080 npm start\`).
It serves the static frontend and the generated REST API + SQLite database.

> Generated file — re-running \`nyx build\` regenerates \`server.js\`. Your edits to this
> README and any custom \`package.json\` fields are preserved.
`;
...
try {
  ...
  console.log(`\n✅ Parsed ${ast.body.length} top-level nodes successfully.`);
    ...
    // === Check ===
    // Validate a .nyx file (imports + parse + semantic validation) WITHOUT emitting
    // anything. Fast feedback for editors, CI, and AI codegen loops.
    //   exit 0 = clean (warnings allowed unless --strict)
    //   exit 1 = errors (or warnings under --strict)
    ...
    if (resolved.errors.length > 0) {
      ...
      console.error(`\n\x1b[31m${resolved.errors.length} import error(s).\x1b[0m`);
      process.exit(1);
    }
    const validator = new Validator();
    const results = validator.validate(resolved.ast, new Set());
    const errors = results.filter(e => e.severity === 'error');
    const warnings = results.filter(e => e.severity === 'warning');
    for (const w of warnings) {
      console.error(`\x1b[33m⚠️  Warning: ${w.message} (line ${w.line}:${w.col})\x1b[0m`);
    }
    ...
    const rel = relative(process.cwd(), filePath);
    if (errors.length > 0) {
      console.error(`\n\x1b[31m✗ ${rel}: ${errors.length} error(s), ${warnings.length} warning(s).\x1b[0m`);
      process.exit(1);
    }
    if (strict && warnings.length > 0) {
      console.error(`\n\x1b[31m✗ ${rel}: ${warnings.length} warning(s) (--strict).\x1b[0m`);
      process.exit(1);
    }
    console.log(`\x1b[32m✓ ${rel}: no errors${warnings.length ? `, ${warnings.length} warning(s)` : ''}.\x1b[0m`);
    ...
    if (tables.length > 0 || apis.length > 0) {
      ...
      // Required npm deps for the generated backend.
      ...
      // Emit a runnable package.json + README so `npm install && npm start` just works.
      // Without this, server.js references deps that are never declared and the app
      // cannot be started without the user hand-writing a manifest. (Issue: generated
      // backend was not directly runnable.)
      emitBackendManifest(outDir, basename(filePath, extname(filePath)), deps);
      console.log(`   📦 deps: ${deps.join(' ')}`);
      console.log(`   🖥️  server.js + package.json generated (${tables.length} table${tables.length !== 1 ? 's' : ''}${security ? ' + auth' : ''})`);
      console.log(`   ▶️  run it: cd ${relative(process.cwd(), outDir) || '.'} && npm install && npm start`);
    }

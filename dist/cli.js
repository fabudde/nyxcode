...
import { resolve, dirname, relative, isAbsolute, join, extname } from 'path';
import { execSync as cpExecSync } from 'child_process';
function safeLoad(absPath, projectRootReal) {
 ...
 */
function decorateError(filePath, source, msg) {
...
function resolveAllImports(entryPath) {
    ...
    const seenPages = new Map(); // route -> file
    const seenComponents = new Map(); // name  -> file
    const seenLayoutFile = { value: null };
    const seenThemeFile = { value: null };
    const mergedBody = [];
    function resolveImportPath(rawPath, fromFile) {
    ...
    function loadFile(absPath, fromFile, rawPath) {
        ...
        if (loaded.kind === 'dir') {
            ...
            if (visited.has(loaded.realPath))
                return;
        ...
        for (const node of imported.body) {
            if (node.type === 'Use' && !node.packageMode) {
                const subResolved = resolveImportPath(node.path, absReal);
                if (subResolved !== null) {
                    loadFile(subResolved, absReal, node.path);
                }
            }
            ...
            if (node.type === 'Theme' && node.extends) {
                const extPath = node.extends;
    ...
    function mergeNodes(nodes, fromFile) {
        const fileRel = relative(projectRoot, fromFile);
        for (const node of nodes) {
            if (node.type === 'Use' && !node.packageMode)
                continue; // file imports already processed
            if (node.type === 'Page') {
 ...
 */
function flattenToSource(entryPath) {
    const entryAbs = resolve(entryPath);
    const projectRoot = dirname(entryAbs);
    let projectRootReal;
    try {
    ...
    const visited = new Set();
    const errors = [];
    const orderedFiles = []; // post-order: deepest dependencies first
    ...
    function resolveInternalPath(rawPath, fromFile) {
        // v0.24.1 (#92): align with resolveAllImports — allowlist, not denylist.
        if (/^[a-zA-Z][a-zA-Z0-9+.-]+:/.test(rawPath) || rawPath.startsWith('//'))
            return null;
        let abs;
        if (rawPath.startsWith('@/'))
            abs = resolve(projectRoot, rawPath.slice(2));
        else if (isAbsolute(rawPath))
            abs = rawPath;
        else
            abs = resolve(dirname(fromFile), rawPath);
    ...
    function walk(absPath, fromFile, rawPath) {
        if (visited.has(absPath))
            return;
        ...
        for (const node of ast.body) {
            if (node.type === 'Use' && !node.packageMode) {
                const sub = resolveInternalPath(node.path, absReal);
                if (sub !== null)
                    walk(sub, absReal, node.path);
            }
        }
    ...
    if (errors.length > 0)
        return { source: '', errors, fileCount: 0 };
 ...
 */
function getDefineFlags(argv) {
    const out = {};
    const stripDunder = (k) => k.replace(/^__|__$/g, '');
    const addKV = (raw) => {
        if (!raw)
            return;
...
if (command === 'theme' && args[1] === 'import') {
    ...
    if (!figmaPath) {
        console.error(`Usage: nyx theme import <tokens.json> [--name <theme-name>] [-o <out.nyx>]\n` +
            `\n` +
    ...
    try {
        ...
        console.error(`\x1b[31m❌ Invalid JSON in ${absTokens}: ${e.message}\x1b[0m`);
        process.exit(1);
    }
    ...
    if (!result.nyx) {
        console.error(`\x1b[31m❌ No tokens could be imported from ${figmaPath}.\x1b[0m`);
        process.exit(1);
    }
    if (outputPath) {
        const absOut = resolve(outputPath);
        writeFileSync(absOut, result.nyx, 'utf-8');
        const totalTokens = Object.values(result.stats).reduce((a, b) => a + b, 0);
...
if (command === 'dev') {
    ...
    const nyxFiles = readdirSync('.').filter((f) => f.endsWith('.nyx'));
        ...
        if (content.includes(useStatement)) {
            ...
            for (let i = 0; i < lines.length; i++) {
                if (lines[i].trim().startsWith('use '))
                    insertIdx = i + 1;
                else if (lines[i].trim().startsWith('theme ') && insertIdx === 0)
                    insertIdx = i;
            }
    ...
    try {
        if (command === 'parse') {
            ...
            if (pages.length <= 1) {
                ...
                if (output.css)
                    console.log(`   CSS:  ${output.css.length} bytes`);
                if (output.js)
                    console.log(`   JS:   ${output.js.length} bytes`);
            }
            else {
            ...
            const security = ast.body.find((n) => n.type === 'Security');
            const config = ast.body.find((n) => n.type === 'Config');
            const hooks = ast.body.filter((n) => n.type === 'Hook');
            const everys = ast.body.filter((n) => n.type === 'Every');
            ...
            if (tables.length > 0 || apis.length > 0) {
                ...
                const protectedPaths = security
                    ...
                    : [];
                const middlewares = ast.body.filter((n) => n.type === 'Middleware');
                const actions = ast.body.filter((n) => n.type === 'Action');
                const envNode = ast.body.find((n) => n.type === 'Env');
                const onEvents = ast.body.filter((n) => n.type === 'OnEvent');
                const useStmts = ast.body.filter((n) => n.type === 'Use' && n.packageMode);
                ...
                if (security) {
                    ...
                    serverCode = serverCode.replace('// ── Create tables', authCode + protectLines + '\n\n// ── Create tables');
                }
                ...
                if (config?.cors)
                    deps.push('cors');
                if (tables.some((t) => t.columns.some((c) => c.type === 'upload')))
                    deps.push('multer');
            ...
            let currentWatchFiles = [filePath];

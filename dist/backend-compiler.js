...
function buildJoinCode(table, allTables) {
    ...
    for (const rel of relations) {
        ...
        for (const rc of refCols)
            fields.push(`${rc.name}: row.__${rel.column}_${rc.name}`);
...
// ── CRUD generator ─────────────────────────────────────────────────────
function generateValidation(columns) {
    const checks = [];
...
function crudForTable(table, allTables, onEvents = []) {
    const n = table.name;
    // Generate on-event hook calls for this table (v0.30)
    const tableEvents = onEvents.filter((e) => e.table === n);
    const createdHook = tableEvents.find((e) => e.event === 'created')
        ? `\n    onEvent_${n}_created(db.prepare('SELECT * FROM ${n} WHERE id = ?').get(info.lastInsertRowid));` : '';
    const updatedHook = tableEvents.find((e) => e.event === 'updated')
        ? `\n    onEvent_${n}_updated(db.prepare('SELECT * FROM ${n} WHERE id = ?').get(req.params.id));` : '';
    const deletedHook = tableEvents.find((e) => e.event === 'deleted')
    ...
    const autoUserCols = userFkCols.map(c => `const ${c.name} = req.user ? req.user.id : req.body.${c.name};\n    `).join('');
...
function compileHook(hook) {
    ...
    for (const stmt of hook.body) {
        if (stmt.type === 'Query') {
            const q = stmt;
            const params = [...q.sql.matchAll(/\$(\w+)/g)].map((m) => m[1]);
...
function compileApiRoute(api) {
    const method = api.method.toLowerCase();
    let middleware = '';
    if (api.guard) {
        middleware = `authMiddleware, roleGuard('${api.guard}'), `;
    }
    else if (api.auth) {
    ...
    for (const stmt of api.body) {
        ...
        if (stmt.type === 'Email')
            emails.push(stmt);
        if (stmt.type === 'ActionCall')
            actionCalls.push(stmt);
    }
    let handlerBody = '';
        ...
        for (const field of v.fields) {
            for (const rule of field.rules) {
                if (rule === 'required') {
                    ...
                    if (fmt === 'email')
                        handlerBody += `    if (req.body.${field.name} && !/^[^\\s@]+@[^\\s@]+\\.[^\\s@]+$/.test(req.body.${field.name})) return res.status(400).json({ error: '${field.name} must be a valid email' });\n`;
                }
            }
        }
    }
    ...
    if (queries.length > 0) {
        ...
        if (r.body && typeof r.body === 'object') {
            ...
            const entries = Object.entries(r.body).map(([k, v]) => {
                if (typeof v === 'object' && v !== null && v.isRef) {
                    return `${JSON.stringify(k)}: ${v.value}`;
                }
...
function compileAction(action) {
    ...
    for (const stmt of action.body) {
        if (stmt.type === 'Let') {
            ...
            const sqlParams = [...stmt.sql.matchAll(/\$([\w.]+)/g)].map((m) => m[1]);
...
// ── Env validation ─────────────────────────────────────────────────────
function compileEnvNode(env) {
    if (!env || !env.vars || env.vars.length === 0)
        return '';
    const checks = [];
...
function compileUseStatements(useStmts) {
    if (!useStmts || useStmts.length === 0)
        return { imports: '', init: '' };
    const imports = [];
    const inits = [];
    for (const u of useStmts) {
        if (!u.packageMode)
            continue; // skip file imports
        if (u.packageMode === 'builtin') {
            const adapter = TIER1_ADAPTERS[u.packageName];
            if (adapter) {
                if (adapter.require)
                    imports.push(adapter.require);
                if (adapter.init)
                    inits.push(adapter.init);
            }
        }
...
function compileOnEvents(onEvents) {
        ...
        for (const stmt of ev.body) {
            if (stmt.type === 'Let') {
                if (stmt.value.kind === 'query') {
                    ...
                    const params = [...stmt.value.sql.matchAll(/\$(\w[\w.]*)/g)].map((m) => m[1]);
                    const paramList = params.map((p) => p.startsWith('row.') ? `row.${p.slice(4)}` : p).join(', ');
                ...
                const safeSql = stmt.sql.replace(/\$(\w[\w.]*)/g, '?');
                const params = [...stmt.sql.matchAll(/\$(\w[\w.]*)/g)].map((m) => m[1]);
                const paramList = params.map((p) => p.startsWith('row.') ? `row.${p.slice(4)}` : p).join(', ');
...
function compileEveryBlocks(everys) {
    if (!everys || everys.length === 0)
        return '';
    const intervals = [];
    const cleanups = [];
 ...
 *   }
 */
function compileEveryBody(stmts) {
    ...
    if (!firstIsSelect) {
        ...
        return stmts.map((s) => '    ' + compileEveryStatement(s)).join('\n');
    }
 ...
 */
function compileEveryStatementInLoop(stmt) {
    if (stmt.type === 'Query') {
      ...
      try {
        ...
        const rowRefs = [...sql.matchAll(/\$row\.(\w+)/g)].map((m) => m[1]);
        if (rowRefs.length > 0) {
            const safeSql = sql.replace(/\$row\.(\w+)/g, '?');
            const params = rowRefs.map((f) => `row.${f}`).join(', ');
...
function compileEveryStatement(stmt) {
    if (stmt.type === 'Query') {
        const sql = stmt.sql || stmt.value || '';
...
function compilePipeStateTable() {
    return `
...
export function compileBackend(tables, apis = [], config = undefined, hooks = [], pagePaths = [], middlewares = [], everys = [], actions = [], envNode, onEvents = [], useStatements = [], pipes = []) {
    ...
    const configPortDefault = config?.envVars?.find((e) => e.name === 'PORT')?.defaultValue;
...
//# sourceMappingURL=backend-compiler.js.map
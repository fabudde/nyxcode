...
export class Validator {
    validate(program, importedComponents) {
        ...
        const usePackages = program.body.filter(n => n.type === 'Use' && n.packageMode).map(n => n.path);
        for (const node of program.body) {
            if (node.type === 'Pipe') {
                const pipe = node;
                ...
                const hasSmsPipe = pipe.steps.some(s => s.type === 'PipeNotify' && s.channel === 'sms');
                ...
                const hasEmailPipe = pipe.steps.some(s => s.type === 'PipeNotify' && s.channel === 'email');
                ...
                const hasRespond = pipe.steps.some(s => s.type === 'PipeRespond');
                ...
                const hasFetchToVar = pipe.steps.some(s => s.type === 'PipeFetch' && s.url && s.url.startsWith('$'));
                if (hasFetchToVar) {
                    errors.push({

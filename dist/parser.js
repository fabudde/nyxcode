 ...
 */
export function isCompileTimeIdent(name) {
    return /^__\w+__$/.test(name);
}
function hasCompileTimeIdentifier(expr) {
...
export class Parser {
    ...
    parseTopLevel() {
        ...
        switch (token.type) {
            ...
            case TokenType.Keyframes: return this.parseTopLevelKeyframes();
            ...
            case TokenType.Identifier:
                if (token.value === 'middleware')
                    return this.parseMiddleware();
                if (token.value === 'meta')
                    return this.parseMeta();
                if (token.value === 'footnotes')
    ...
    parseEvery() {
        ...
        if (!match)
            throw this.error(`Invalid interval '${interval}'. Use format: 30s, 5m, 1h`);
    ...
    parsePipeValidate() {
        ...
        return { type: 'PipeValidate', fields, line: this.peek().line, col: this.peek().col };
    }
    ...
    parsePipeQuery() {
        ...
        return { type: 'PipeQuery', sql, as: asVar, line: this.peek().line, col: this.peek().col };
    }
    ...
    parsePipeSet() {
        ...
        return { type: 'PipeSet', name, expression, line: this.peek().line, col: this.peek().col };
    }
    ...
    parsePipeTransform() {
        ...
        this.consume(TokenType.RightBrace);
        return { type: 'PipeTransform', fields, line: this.peek().line, col: this.peek().col };
    }
    ...
    parsePipeEach() {
        ...
        this.consume(TokenType.RightBrace);
        return { type: 'PipeEach', collection, itemName, body, line: this.peek().line, col: this.peek().col };
    }
    ...
    parsePipeWhen() {
        ...
        return { type: 'PipeWhen', condition: condition.trim(), body, elseBody, line: this.peek().line, col: this.peek().col };
    }
    ...
    parsePipeNotify() {
        ...
        return { type: 'PipeNotify', channel, params, line: this.peek().line, col: this.peek().col };
    }
    ...
    parsePipeAbort() {
        ...
        return { type: 'PipeAbort', status, message, line: this.peek().line, col: this.peek().col };
    }
    // webhook "url" [body={...}]
    parsePipeWebhook() {
        this.advance(); // consume 'webhook'
        ...
        return { type: 'PipeWebhook', url, body, line: this.peek().line, col: this.peek().col };
    }
    ...
    parsePipeRun() {
        ...
        return { type: 'PipeRun', pipeName, withParams, line: this.peek().line, col: this.peek().col };
    }
    // Helper: consume { key: value, key2: value2 } as a raw key-value map
    consumePipeInlineObject() {
        this.consume(TokenType.LeftBrace);
        const obj = {};
        while (!this.check(TokenType.RightBrace) && !this.isAtEnd()) {
            while (this.check(TokenType.Newline) || this.check(TokenType.Comma))
    ...
    parseApi() {
        ...
        const body = this.parseBody();
        this.consume(TokenType.RightBrace);
        return { type: 'Api', method, path, body, auth, guard, middleware: middlewareNames.length > 0 ? middlewareNames : undefined, line: start.line, col: start.col };
    }
    parseHook(timing) {
    ...
    parseStore() {
        ...
        while (!this.check(TokenType.RightBrace) && !this.isAtEnd()) {
            ...
            if (this.peek().type === TokenType.Computed) {
                ...
                while (!this.isAtEnd()) {
                    const next = this.peek();
                    if (next.type === TokenType.RightBrace && parenDepth === 0)
                        break;
                    ...
                    if (parenDepth === 0 && (next.type === TokenType.State || next.type === TokenType.Computed ||
                        (next.type === TokenType.Identifier && this.peekAt(1)?.type === TokenType.Equals)))
                        break;
    ...
    parseTheme() {
        ...
        if (this.check(TokenType.String)) {
                ...
                while (!this.check(TokenType.RightBrace) && !this.isAtEnd()) {
                    ...
                    while (!this.check(TokenType.RightBrace) && !this.isAtEnd()) {
                        ...
                        if (this.peek().type === TokenType.String)
                            ...
                            while (!this.check(TokenType.RightBrace) && !this.check(TokenType.Identifier) && !this.isAtEnd())
                                val += this.advance().value;
            ...
            return { type: 'Theme', preset, sections, line: start.line, col: start.col };
        }
        ...
        while (!this.check(TokenType.RightBrace) && !this.isAtEnd()) {
            ...
            if (sectionName === 'icons') {
                if (this.check(TokenType.Colon))
                    this.advance(); // skip optional colon
            ...
            if (sectionName === 'defaults') {
                ...
                while (!this.check(TokenType.RightBrace) && !this.isAtEnd()) {
                    ...
                    this.consume(TokenType.RightBrace);
                    if (props.length > 0)
                        defs.push({ element, properties: props });
                }
                this.consume(TokenType.RightBrace);
            ...
            while (!this.check(TokenType.RightBrace) && !this.isAtEnd()) {
                ...
                if (this.peek().type === TokenType.String) {
                    ...
                    while (sectionName === 'fonts' && this.check(TokenType.Comma)) {
                        ...
                        if (la.type === TokenType.Identifier && la.value === 'source')
                            break;
                        if (la.type === TokenType.RightBrace)
                            break;
                    ...
                    if (sectionName === 'fonts' && this.check(TokenType.Identifier) && this.peek().value === 'source') {
                        ...
                        if (sourceType === 'url') {
                            ...
                            throw new Error(`[NyxCode Parser Error] External URL font sources are deprecated for security. ` +
                ...
                if (sectionName === 'fonts') {
                    ...
                    const flushPart = () => { if (currentParts.length) {
            ...
            if (Object.keys(fontsMeta).length > 0)
                section.fontsMeta = fontsMeta;
            sections.push(section);
        }
        this.consume(TokenType.RightBrace);
        const node = { type: 'Theme', sections, line: start.line, col: start.col };
        if (mode)
            node.mode = mode;
        if (themeName)
            node.name = themeName;
        if (themeExtends)
            node.extends = themeExtends;
        if (bodyStyles && bodyStyles.length > 0)
            node.body = bodyStyles;
        if (selectionStyles && selectionStyles.length > 0)
            node.selection = selectionStyles;
        if (defaultsList && defaultsList.length > 0)
            node.defaults = defaultsList;
        if (iconsConfig)
            node.icons = iconsConfig;
    ...
    parseSecurity() {
        ...
        while (!this.check(TokenType.RightBrace) && !this.isAtEnd()) {
            ...
            while (!this.check(TokenType.RightBrace) && !this.isAtEnd()) {
                ...
                if (values.length > 0 && (next.type === TokenType.Identifier && ruleKeywords.includes(next.value)))
                    break;
                if (values.length > 0 && next.value.startsWith('/'))
                    break;
                ...
                while (!this.check(TokenType.RightBrace) && !this.isAtEnd()) {
                    ...
                    while (!this.check(TokenType.RightBrace) && !this.isAtEnd()) {
                        ...
                        if (val && t.type === TokenType.Identifier && /^[a-z]/.test(t.value) && this.peekAt(1)?.type !== TokenType.RightBrace) {
                            ...
                            if (next && (next.type === TokenType.Colon || next.type === TokenType.Identifier || next.type === TokenType.Number || (next.value && next.value.startsWith('#'))))
                                break;
                        }
                        val += (val ? ' ' : '') + this.advance().value;
                    }
                    if (val)
                        style.push({ name: prop, value: val });
                }
            ...
            case TokenType.Identifier:
                ...
                if ((this.peek().value === 'onMount' || this.peek().value === 'onDestroy') && this.peekAt(1)?.type === TokenType.LeftBrace) {
                    ...
                    if (this.peek().type === TokenType.LeftBrace) {
                        ...
                        return { type: 'Script', content: `__nyx_${hookName}:${body.trim()}`, line: this.tokens[this.pos - 1].line, col: this.tokens[this.pos - 1].col };
                    }
                    return this.parseElement();
                }
                return this.parseElement();
    ...
    parseData() {
            ...
            while (!this.check(TokenType.RightBrace) && !this.isAtEnd()) {
                ...
                if (keyword.type === TokenType.Identifier && ['loading', 'error', 'empty'].includes(keyword.value)) {
                    ...
                    if (this.check(TokenType.LeftBrace)) {
                        ...
                        const elemNode = { type: 'Element', tag: elemTag, content: elemContent, attributes: [], children: [], line: start.line, col: start.col };
                        if (which === 'loading')
                            loadingBlock = [elemNode];
                        else if (which === 'error')
                            errorBlock = [elemNode];
                        else if (which === 'empty')
                            emptyBlock = [elemNode];
        ...
        while (this.check(TokenType.Identifier) && this.peek().value === 'catch') {
            ...
            while (!this.isAtEnd()) {
                if (this.peek().type === TokenType.RightBrace)
                    break;
                ...
                if (pt === TokenType.Data || pt === TokenType.Each || pt === TokenType.When ||
                    pt === TokenType.Form || pt === TokenType.State || pt === TokenType.Style)
                    break;
    ...
    parseDataSource() {
        ...
        const kind = kindToken.value.toLowerCase();
        if (kind === 'query') {
            const sql = this.consume(TokenType.String).value;
     ...
     */
    static COMMA_VALUE_PROPERTIES = new Set([
        // Full CSS property names
    ...
    isCssPseudoSelector() {
        ...
        if (this.pos + 2 >= this.tokens.length || this.tokens[this.pos + 2].type !== TokenType.Identifier)
            return false;
        // Then eventually a {
        let i = this.pos + 3;
        while (i < this.tokens.length && this.tokens[i].type !== TokenType.LeftBrace && this.tokens[i].type !== TokenType.RightBrace)
            i++;
    ...
    parseStyleProperty() {
        ...
        while (!this.isAtEnd()) {
            ...
            if (next.type === TokenType.GreaterThan || next.value === '~' || next.value === '+')
                break;
            ...
            if (['hover', 'focus', 'active', 'before', 'after', 'first-child', 'last-child', 'disabled', 'checked', 'focus-within', 'focus-visible', 'placeholder', 'empty', 'visited'].includes(next.value))
                break;
            ...
            if (next.type === TokenType.Identifier && value.length > 0) {
                ...
                if (peek1?.value === '-' && peek2?.type === TokenType.Identifier) {
                    ...
                    if (Parser.CSS_PROPERTIES.has(hyphenated))
                        break;
                }
                if (Parser.CSS_PROPERTIES.has(next.value))
                    break;
            }
    ...
    parseForm() {
        ...
        while (!this.check(TokenType.RightBrace) && !this.isAtEnd()) {
            if (this.check(TokenType.Identifier) && (this.peek().value === 'success' || this.peek().value === 'error')) {
                ...
                this.consume(TokenType.Arrow);
                const actionKind = this.advance().value;
                let actionValue;
                ...
                const formAction = { kind: actionKind, value: actionValue };
        ...
        while (!this.check(TokenType.RightBrace) && !this.isAtEnd()) {
            ...
            if (next.type === TokenType.Identifier && parts.length > 0) {
                ...
                if (after && after.type === TokenType.LeftParen) { /* function call, continue */ }
                    ...
                    break; // new line + bare identifier = next property name
            }
            parts.push({ kind: 'token', value: this.advance().value });
            lastLine = next.line;
        }
    ...
    parseComputed() {
        ...
        while (!this.isAtEnd()) {
            ...
            if (parenDepth === 0 && next.type === TokenType.Identifier && ELEMENT_TAGS.has(next.value))
                break;
            ...
            ].includes(next.type) && expression.length > 0 && !expression.endsWith('.'))
                break;
            if (next.type === TokenType.LeftParen)
                parenDepth++;
            if (next.type === TokenType.RightParen)
    ...
    looksLikeStyleBlock() {
        ...
        if (after.type === TokenType.Identifier && ['hover', 'focus', 'active'].includes(after.value))
            return true;
        return false;
    }
    // Parse { fs 1.3rem, c text } as a StyleBlock (without 'style' keyword)
    parseStyleBlockInline() {
        ...
        let focus;
        let active;
        const PSEUDO_CLASSES = { 'hover': 'hover', 'focus': 'focus', 'active': 'active' };
        while (!this.check(TokenType.RightBrace) && !this.isAtEnd()) {
            // Skip commas
            if (this.check(TokenType.Comma)) {
                this.advance();
    ...
    parseElement() {
        ...
        while (!this.isAtEnd() && !this.check(TokenType.RightBrace)) {
                    ...
                    while (!this.isAtEnd() && !this.isStatementStart()) {
                        ...
                        if (cur.type === TokenType.LeftBrace) {
                            if (action.trim().length > 0)
                                break; // Action complete — { starts element children
                            action += ' {';
                            this.advance();
                            let depth = 1;
                            while (depth > 0 && !this.isAtEnd()) {
            ...
            // Property access: .name or .author.name (nested)
                ...
                if (content && content.type === 'Identifier') {
                    const storeName = content.name;
                    const field = path.substring(1); // remove leading dot
                    content = { type: 'StoreAccess', store: storeName, field, line: next.line, col: next.col };
                }
                else {
                    ...
                    while (!this.check(TokenType.RightBrace) && !this.isAtEnd()) {
                        ...
                        while (!this.isAtEnd()) {
                            ...
                            if (parenD > 0) {
                                ...
                                else if ((tok.value === '-' || tok.value === '+') && propVal.length > 0 && !propVal.endsWith('(')) {
                                    propVal += ' ' + tok.value + ' ';
                                }
                                else {
            ...
            // Children/Style block: { ... }
                ...
                if (content && content.type === 'PropertyAccess' && this.looksLikeStyleBlock()) {
    ...
    parseProps() {
        ...
        while (this.check(TokenType.Identifier)) {
            ...
            if (ELEMENT_TAGS.has(this.peek().value))
                break;
            // Stop if the identifier is uppercase (component invocation)
            if (this.peek().value[0] >= 'A' && this.peek().value[0] <= 'Z')
                break;
            const name = this.consumeIdentifier();
            ...
            if (this.check(TokenType.Colon)) {
                this.advance(); // :
                if (this.check(TokenType.Identifier))
                    this.advance(); // the type keyword (string, number, bool, etc.)
            }
            let defaultValue;
     ...
     */
    isColumnType(token, typeKeywords) {
